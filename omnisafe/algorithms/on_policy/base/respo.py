...
from omnisafe.adapter import OnPolicyAdapterRespo
from omnisafe.algorithms import registry
from omnisafe.algorithms.base_algo import BaseAlgo
from omnisafe.common.lagrange import Lagrange
from omnisafe.common.buffer import VectorOnPolicyRespoBuffer
from omnisafe.common.logger import Logger
from omnisafe.models.actor_critic.constraint_actor_multiple_critic import ConstraintActorMultipleCritic
from omnisafe.utils import distributed


@registry.register
# pylint: disable-next=too-many-instance-attributes,too-few-public-methods,line-too-long
class RESPO(BaseAlgo):
    """Respo

    References:
        - https://arxiv.org/pdf/2309.13528.pdf
        - https://github.com/milanganai/milanganai.github.io/blob/main/NeurIPS2023/code/respo.py
    """
    ...
    def _init_env(self) -> None:
        ...
        Raises:
            AssertionError: If the number of steps per epoch is not divisible by the number of
                environments.
        """
        # need custom Respo Adapter to also log value_p
        self._env: OnPolicyAdapterRespo = OnPolicyAdapterRespo(
            self._env_id,
    ...
    def _init_model(self) -> None:
        ...
        """
        self._actor_critic: ConstraintActorMultipleCritic = ConstraintActorMultipleCritic(
    ...
    def _init(self) -> None:
        ...
        """
        self._buf: VectorOnPolicyRespoBuffer = VectorOnPolicyRespoBuffer(
            obs_space=self._env.observation_space,
            act_space=self._env.action_space,
            size=self._steps_per_epoch,
            gamma=self._cfgs.algo_cfgs.gamma,
            gamma_prob=self._cfgs.algo_cfgs.gamma_prob,
    ...
    def _update(self) -> None:
        ...
        value_p, discounted_prob = data['value_p'], data['discounted_prob']

        original_obs = obs
        old_distribution = self._actor_critic.actor(obs)

        dataloader = DataLoader(
            dataset=TensorDataset(obs, act, logp, target_value_r, target_value_c, adv_r, adv_c, value_p, discounted_prob),
        ...
        for i in track(range(self._cfgs.algo_cfgs.update_iters), description='Updating...'):
            for (
                ...
                adv_c,
                value_p, 
                discounted_prob # target_value_p
            ) in dataloader:
                self._update_reward_critic(obs, target_value_r)
                if self._cfgs.algo_cfgs.use_cost:
                    self._update_cost_critic(obs, target_value_c)
                    self._update_prob_critic(obs, discounted_prob)
    ...
    def _update_prob_critic(self, obs: torch.Tensor, target_value_p: torch.Tensor) -> None:
        ...
        loss = nn.functional.mse_loss(self._actor_critic.prob_critic(obs)[0], target_value_p)
        ...
        distributed.avg_grads(self._actor_critic.prob_critic)
    ...
    def _update_actor(  # pylint: disable=too-many-arguments
        ...
        """
        adv = self._compute_adv_surrogate(adv_r, adv_c, value_p)
    ...
    def _compute_adv_surrogate(  # pylint: disable=unused-argument
        ...
        unsafe = value_p
        safe = 1.0 - value_p
        return (adv_r * safe - penalty * adv_c * safe - unsafe * adv_c) / (1 + penalty)
...
#             | target_value_c | ``target cost value`` sampled from buffer.                       |
#             +----------------+------------------------------------------------------------------+
#             | target_value_c | ``target prob value`` sampled from buffer.                       |
#             +----------------+------------------------------------------------------------------+
#             | logp           | ``log probability`` sampled from buffer.                         |
#             +----------------+------------------------------------------------------------------+
#             | adv_r          | ``estimated advantage`` (e.g. **GAE**) sampled from buffer.      |
...
#         """
#         data = self._buf.get()
#         obs, act, logp, target_value_r, target_value_c, target_value_p, adv_r, adv_c = (
#             data['obs'],
#             data['act'],
#             data['logp'],
#             data['target_value_r'],
...
#         return (adv_r * prob_safe - penalty * adv_c) / (1 + penalty)
...
#             logp (torch.Tensor): The ``log probability`` of action sampled from buffer.
#             adv (torch.Tensor): The ``advantage`` processed. ``reward_advantage`` here.
#             prob_value (torch.Tensor): The ``probability`` of entering an unsafe region sampled from buffer.
