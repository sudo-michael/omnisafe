...
class ALMPG(BaseAlgo):
    ...
    def _init_log(self) -> None:
        ...
        self._logger.register_key("Metrics/AugmentedLagrangianRegularizer")
        self._logger.register_key("Metrics/UseGeneralUtilityRewards")
    ...
    def _update(self) -> None:
        ...
        dataloader = DataLoader(
            dataset=TensorDataset(
                obs, act, logp, target_value_r, target_value_c, adv_r, adv_gur
            ),
    ...
    def _update_actor(  # pylint: disable=too-many-arguments
        ...
        """
        if use_general_utility := self._use_general_utility_rewards():
            adv = adv_gur
        else:
            adv = adv_r
    ...
    def _loss_pi(
        ...
        Args:
            obs (torch.Tensor): The ``observation`` sampled from buffer.
            act (torch.Tensor): The ``action`` sampled from buffer.
            log_pi_t (torch.Tensor): The ``log probability`` of action sampled from buffer.
            adv (torch.Tensor): The ``advantage`` processed. ``reward_advantage`` here.

        Returns:
            The loss of pi/actor.
        """
        # compute surrogate based on functional mirror ascent with the softmax representation and log-sum-exp mirror map
        # Appendix D. https://openreview.net/pdf/676829ee4fad5f8c74b44ca4d199d180b49db5df.pdf
