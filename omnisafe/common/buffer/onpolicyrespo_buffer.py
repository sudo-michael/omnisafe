...
class OnPolicyRespoBuffer(BaseBuffer):  # pylint: disable=too-many-instance-attributes
    ...
    +----------------+---------+---------------+----------------------------------------+
    | discounted_prob| (size,) | torch.float32 | The discounted sum of return.          |
    +----------------+---------+---------------+----------------------------------------+
    ...
    +----------------+---------+---------------+----------------------------------------+
    | value_p        | (size,) | torch.float32 | The value estimated by prob critic.    |
    +----------------+---------+---------------+----------------------------------------+
    ...
    Args:
        ...
        lam_p (float): The lambda factor for calculating the advantages of the critic.
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        self.data['adv_p'] = torch.zeros((size,), dtype=torch.float32, device=device)
        ...
        self._gamma_prob: float = gamma_prob
        self._lam: float = lam
    ...
    def finish_path(
        ...
        last_value_p = (last_value_c > 0.0).to(torch.float32)
        ...
        probs = torch.cat([self.data['prob'][path_slice], last_value_p])
        ...
        def respo_prob(vector_x, gamma):
            length = vector_x.shape[0]
            for idx in reversed(range(length - 1)):
                vector_x[idx] = max(vector_x[idx] , gamma * vector_x[idx+1])
            return vector_x

        discounted_prob = respo_prob(probs, self._gamma_prob)[:-1]
        self.data['discounted_prob'][path_slice] = discounted_prob
