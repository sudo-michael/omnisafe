...
class OnPolicyALMBuffer(BaseBuffer):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        self.data["target_value_c"] = torch.zeros(
            (size,), dtype=torch.float32, device=device
        )
        ...
        assert self._penalty_coefficient >= 0, (
            "penalty_coefficient must be non-negative!"
        )
    ...
    def _calculate_v_trace(
        ...
        assert behavior_action_probs.ndim == 1, (
            "Please provide arrays instead of scalars"
        )
        ...
        for index in reversed(range(sequence_length)):
            delta = clip_rhos[index] * (
                rewards[index] + gamma * values[index + 1] - values[index]
            )
            v_s[index] += delta + gamma * clip_cs[index] * (
                last_v_s - values[index + 1]
            )
